...
const tabContent: Record<string, { icon: any; title: string; description: string }[]> = {
  ...
  automation: [
    { icon: Globe, title: "AI Browser", description: "AI-native browser with multi-step task execution across websites" },
...
const SolutionsSection = () => {
  ...
  return (
    <section id="solutions" className="section-padding relative overflow-hidden bg-[#fafafa]">
      {/* Refined Background Gradient - Less "Orange Orange", more subtle warmth */}
        ...
        >
          <div className="flex justify-center mb-6">
            <span className="px-5 py-2 rounded-full bg-primary/10 backdrop-blur-md text-primary text-[10px] font-bold tracking-[0.2em] uppercase border border-primary/20 shadow-[0_0_20px_rgba(249,115,22,0.1)]">
              <span className="opacity-40 mr-1">「</span> How it works <span className="opacity-40 ml-1">」</span>
            </span>
          </div>
          <h2 className="font-heading text-3xl sm:text-5xl lg:text-7xl font-bold text-foreground max-w-4xl mx-auto leading-[1.2] lg:leading-[1.1] tracking-tight">
            How Our AI Brings <GradientText colors={["#f97316", "#fb923c", "#f97316"]} className="italic font-light pr-4">Your</GradientText> <br className="hidden sm:block" /> Vision To Life
          </h2>
        </motion.div>

        {/* Tabs - True Glassmorphism (Blur + Translucent + Neutral Text) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          ...
          {tabs.map((tab, i) => (
            ...
            >
              <span className="flex items-center gap-2 sm:gap-3">
                <span className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] shrink-0 ${activeTab === tab.id ? "bg-black/5" : "bg-black/5"
                  }`}>
                  {i + 1}.
                </span>
                <span className="whitespace-nowrap">{tab.label.toUpperCase()}</span>
              </span>
            </button>
          ))}
        </motion.div>
        ...
        >
          {tabContent[activeTab]?.map((item, idx) => (
            ...
            >
              ...
              <p className="text-sm text-primary/70 leading-relaxed group-hover:text-foreground/80 transition-colors">
                {item.description}
              </p>
            </div>
          ))}
